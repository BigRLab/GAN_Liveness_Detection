...
# get requested normalization layer
def get_norm_layer(norm_type):
    if norm_type == 'batch':
        ...
        print('normalization layer [{}] is not found'.format(norm))
    return norm_layer
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[]):
    ...
    if which_model_netG == 'unet_128':
        ...
        print('Generator model name [{}] is not recognized'.format(which_model_netG))
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    if which_model_netD == 'basic':
        ...
        print('Discriminator model name [{}] is not recognized'.format(which_model_netD))
...
# helper function for printing network structure
...
class GANLoss(nn.Module):
    ...
    # Guess: obtain the synthesized label tensor
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            ...
            if create_label:
                real_tensor = self.Tensor(input.size()).fill_(self.real_label).cuda(device=self.gpu_ids[0])
            ...
            if create_label:
                fake_tensor = self.Tensor(input.size()).fill_(self.fake_label).cuda(device=self.gpu_ids[0])
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc,
        ...
        # inner_nc varies for upconv at different levels
        if outermost:
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]):
        ...
        nf_mult = 1
        nf_mult_prev = 1 # controls the ratio of number of channels between conv_layers
